...
if($ref != ''){
	$head_sql = "SELECT a.voucher_no,a.trans_dated,a.ref_id,a.narration,a.debit_amount,
		...
		jo.job_name,cust.cust_name,co.comp_name
		FROM vouchers a
		...
		LEFT JOIN job_order jo ON a.ref_column='job_order' AND a.ref_id=jo.jd_id
		LEFT JOIN customers cust ON jo.customer_id=cust.cust_id
		WHERE a.cancel_flag=0 AND a.v_type_id=3 AND a.debit_amount>0 AND md5(a.voucher_no)='".$ref."' LIMIT 1";
	$head_qry = $conn->query($head_sql);
	if($head_qry && $head_qry->num_rows > 0){
		$voucher = $head_qry->fetch_assoc();
		$detail_qry = $conn->query("SELECT v.account_id,v.debit_amount,v.credit_amount,a.acc_name FROM vouchers v LEFT JOIN accounts a ON v.account_id=a.account_no WHERE v.cancel_flag=0 AND v.v_type_id=3 AND md5(v.voucher_no)='".$ref."' ORDER BY v.debit_amount DESC");
...
if(!$voucher){ die('Sales invoice not found.'); }
...
<head>
	<meta charset="utf-8">
	<title>Sales Invoice SI-<?php echo $voucher['voucher_no']; ?></title>
	<style>
...
<body>
	<div class="printbar"><button class="printbtn" onclick="window.print()">Print Invoice</button></div>
	<div class="receipt">
		<div class="top">
			<div>
			...
			<div class="title">
				<h1>Sales Invoice</h1>
				<div class="badge">SI-<?php echo $voucher['voucher_no']; ?></div>
			</div>
		</div>
		<div class="meta">
			...
			<div class="box" style="grid-column:span 2"><span>Job</span><strong><?php echo htmlspecialchars($voucher['job_name'] ?: ('Job #'.$voucher['ref_id'])); ?></strong></div>
		</div>
		<div class="content">
			<table>
