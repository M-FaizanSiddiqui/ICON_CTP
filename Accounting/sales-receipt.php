...
$module_qry = $conn->query("SELECT m_id FROM modules_1 WHERE m_url = 'Accounting/sales-receipt' LIMIT 1");
...
if($sales_receipt_module_id == "0" || in_array($sales_receipt_module_id,$_SESSION['login_Permisions']))
{
	...
	$receipt_sql = "SELECT d.voucher_no,d.trans_dated,d.ref_id,d.narration,d.account_id AS deb_acc,d.debit_amount AS amount,
		...
		cp.reference,cp.payment_mode,cp.cheque_no,cp.cheque_date,cust.cust_name
		FROM vouchers d
		...
		LEFT JOIN customer_payment cp ON d.ref_column = 'customer_payment' AND d.ref_id = cp.pay_id
		LEFT JOIN customers cust ON cp.customer_id = cust.cust_id
		WHERE d.cancel_flag = 0 AND d.v_type_id = 4 AND d.debit_amount > 0
		AND ca.company_id = ".$company_id_acc."
				...
				<div class="receipt-hero-content">
					<div class="receipt-title-wrap">
						...
						<div class="receipt-title-copy">
							...
							<p>Customer payment receipts for <?php echo htmlspecialchars($company_name); ?>.</p>
						</div>
					</div>
				</div>
			</div>
			<div class="receipt-card">
				<div class="receipt-toolbar">
					<form class="receipt-filters" method="get" action="index.php">
						<input type="hidden" name="page" value="Accounting/sales-receipt">
					...
					<div class="receipt-stats">
						<div class="receipt-stat"><span>Receipts</span><strong><?php echo number_format($total_count); ?></strong></div>
						<div class="receipt-stat"><span>Total Received</span><strong><?php echo number_format($total_amount,2); ?></strong></div>
					</div>
				</div>
				<div class="receipt-table-wrap">
					<div class="receipt-table-responsive">
						<table class="table receipt-table">
							<thead><tr><th>Receipt No</th><th>Date</th><th>Customer / Reference</th><th>Received In</th><th>Customer Account</th><th style="text-align:right">Amount</th><th>Action</th></tr></thead>
							<tbody>
								<?php if($receipts && $receipts->num_rows > 0){ while($row = $receipts->fetch_assoc()){ ?>
									<tr>
										<td><span class="receipt-code">SR-<?php echo $row['voucher_no']; ?></span></td>
										<td><?php echo date('d-M-Y',strtotime($row['trans_dated'])); ?></td>
										<td><?php echo htmlspecialchars($row['cust_name'] ?: 'Customer'); ?><span class="receipt-account"><?php echo htmlspecialchars($row['reference'] ?: ('Payment #'.$row['ref_id'])); ?></span></td>
										...
										<td><div class="receipt-actions"><a class="receipt-action view" title="View Receipt" href="index.php?page=Accounting/voucher-view&type=sales_receipt&ref=<?php echo md5($row['voucher_no']); ?>"><i class="fa fa-eye"></i></a><a class="receipt-action" target="_blank" title="Print Receipt" href="Accounting/sales-receipt-print.php?ref=<?php echo md5($row['voucher_no']); ?>"><i class="fa fa-print"></i></a></div></td>
									</tr>
								<?php }}else{ ?>
									<tr><td colspan="7" class="receipt-empty">No sales receipts found for selected filters.</td></tr>
								<?php } ?>
							</tbody>
						</table>
					</div>
