...
$module_qry = $conn->query("SELECT m_id FROM modules_1 WHERE m_url = 'Accounting/purchase-receipt' LIMIT 1");
$purchase_receipt_module_id = ($module_qry && $module_qry->num_rows > 0) ? (string)$module_qry->fetch_assoc()['m_id'] : "0";

if($purchase_receipt_module_id == "0" || in_array($purchase_receipt_module_id,$_SESSION['login_Permisions']))
{
	...
	$receipt_sql = "SELECT d.voucher_no,d.trans_dated,d.ref_id,d.ref_column,d.narration,d.account_id AS deb_acc,d.debit_amount AS amount,
		...
		WHERE d.cancel_flag = 0 AND d.v_type_id = 1 AND d.debit_amount > 0
		...
		<div class="receipt-shell">
			...
			<div class="receipt-card">
				<div class="receipt-toolbar">
					...
					<div class="receipt-stats"><div class="receipt-stat"><span>Receipts</span><strong><?php echo number_format($total_count); ?></strong></div><div class="receipt-stat"><span>Total Purchase</span><strong><?php echo number_format($total_amount,2); ?></strong></div></div>
				</div>
				<div class="receipt-table-wrap"><div class="receipt-table-responsive"><table class="table receipt-table">
					<thead><tr><th>Receipt No</th><th>Date</th><th>Reference</th><th>Debit</th><th>Credit</th><th style="text-align:right">Amount</th><th>Action</th></tr></thead>
					<tbody>
						<?php if($receipts && $receipts->num_rows > 0){ while($row = $receipts->fetch_assoc()){ ?>
							<tr>
								<td><span class="receipt-code">PR-<?php echo $row['voucher_no']; ?></span></td>
								<td><?php echo date('d-M-Y',strtotime($row['trans_dated'])); ?></td>
								<td><?php echo htmlspecialchars($row['ref_column'] ?: 'Purchase'); ?><span class="receipt-account">Ref #<?php echo htmlspecialchars($row['ref_id']); ?></span></td>
								...
								<td><div class="receipt-actions"><a class="receipt-action view" title="View Receipt" href="index.php?page=Accounting/voucher-view&type=purchase_receipt&ref=<?php echo md5($row['voucher_no']); ?>"><i class="fa fa-eye"></i></a><a class="receipt-action" target="_blank" title="Print Receipt" href="Accounting/purchase-receipt-print.php?ref=<?php echo md5($row['voucher_no']); ?>"><i class="fa fa-print"></i></a></div></td>
							</tr>
						<?php }}else{ ?><tr><td colspan="7" class="receipt-empty">No purchase receipts found for selected filters.</td></tr><?php } ?>
					</tbody>
				</table></div></div>
			</div>
		</div>
