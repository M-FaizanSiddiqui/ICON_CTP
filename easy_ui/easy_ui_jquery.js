...
$.extend($.fn.validatebox.defaults.rules, {
	date: {
		...
		message: 'Enter In Format(YYYY-MM-DD)'    
	}
});  
...
function no_record_found_func()
{
	var myview = $.extend({},$.fn.datagrid.defaults.view,{
		onAfterRender(target){
			...
			var acb = $(target).datagrid('getRows').length;
			if (!$(target).datagrid('getRows').length)
			{
				var d = $('<div class="datagrid-empty" style="font-size:14px;"></div>').html('No Record Found').appendTo(vc);

				$('.datagrid-view').addClass('no_record_height');
				d.css({
					position:'absolute',left:0,width:'100%',textAlign:'center',color:'red',
				});
			}
			else
			{
				$('.datagrid-view').removeClass('no_record_height');
			}
		}
	});
	return myview;
